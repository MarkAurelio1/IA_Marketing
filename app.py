...
def llm_generate(llm, prompt):
  template = ChatPromptTemplate.from_messages([
      'system', 'Voce é um especialista em marketing digital com foco em SEO e escrita persuasiva'
      'human', '{prompt}',
  ])
...
hashtags = st.checkbox('Retornar Hastags')
...
if st.button('Gerar conteúdo'):
    ...
    - Comprimento: {length}.
    - {'Inclua uma chamda para ação clara.' if cta else 'Não inclua chamada para ação'}
